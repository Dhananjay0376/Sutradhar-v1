...
export function useModelLoader(category: ModelCategory, coexist = false): ModelLoaderResult {
  ...
  const ensure = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      // Prefer the 1.2B Tool model for structured JSON output (calendar/post generation)
      // Fall back to first available model if not found
      const model = models.find(m => m.id === 'lfm2-1.2b-tool-q4_k_m') || models[0];
